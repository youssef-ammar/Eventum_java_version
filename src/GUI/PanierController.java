...
public class PanierController implements Initializable {
 ...
 Commande commande = null;
    ...
    public void refreshlist() {
        ...
        tutorial_col.setCellValueFactory(new PropertyValueFactory<>("produit_id"));
    ...
    private void loadData() {
        ...
        data = FXCollections.observableArrayList(ps.getPanier(1));
        tutorial_col.setCellValueFactory(new PropertyValueFactory<>("produit_id"));
        tutorial_col.setCellValueFactory(cellData -> new SimpleStringProperty(ts.getProduit(cellData.getValue().getproduit_id())));
        int prix = ts.getProduitPrix(2); // yelzem nrecuperi id tutorial 
        quantite_col.setCellValueFactory(new PropertyValueFactory<>("qte"));
                    ...
                    if (empty) {
                        ...
                        deleteIcon.setOnMouseClicked((MouseEvent event) -> {
                            ...
                            try {
                                ...
                                PanierService ps = new PanierService();
                                ps.supprimerPanierId((int) panier.getId());
                                double count = ps.sommePanier(1);
                                totalpanier.setText(String.valueOf(count));
                                refreshlist();
                            } catch (Exception ex) {
...
// add product in cart shop
    ...
    private void ajouter_panier(MouseEvent event) {
        ...
        if (errors.length() > 0) {
            ...
            int prix = ts.getProduitPrix(idproduit);
...
// proceed command
    ...
    private void ajouter_commande(MouseEvent event) {
      //  commande = tableviewcommande.getSelectionModel().getSelectedItem();
        ...
        ps.supprimerPanier();//ifasa5 l panier
